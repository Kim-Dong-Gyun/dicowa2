                    ...
                    <table class="event-board">
                        ...
                        <c:set var = "num" value="${pageMaker.totalCount - (pageMaker.cri.page-1) * pageMaker.cri.perPageNum }"/>
	                        ...
	                        <tbody class="tblbody">
	                            <tr>
	                                <td>${num}</td>
	                                <td><a href="/eventRead?bno=${boardDto.bno}">${boardDto.title }</a></td>
                    ...
                    <div class="bottom">
                        <s_paging>
                            ...
                            <c:if test="${pageMaker.prev }">
                            	<a href="eventList${pageMaker.makeSearch(pageMaker.startPage - 1)}" class="prev" title="이전페이지">◀ 이전 </a>
                            </c:if>
                            <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
                            	<s_paging_rep><a href="eventList${pageMaker.makeSearch(idx)}" class="num">${idx}</a></s_paging_rep>
                            </c:forEach>
                            <c:if test="${pageMaker.next && pageMaker.endPage >0 }">
                            	<a href="eventList${pageMaker.makeSearch(pageMaker.endPage +1)}" class="prev" title="다음페이지">다음 ▶</a>
                            </c:if>
                            <br />&nbsp;
                            </div>
                        </s_paging>
